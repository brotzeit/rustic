...
(defmacro rustic--with-temp-process-buffer (&rest body)
  "Like `with-temp-buffer', but propagate environment and `exec-path'.
Always propagate `process-environment' and `exec-path'.  When those
vars are buffer-local in the calling buffer, they are not propagated
by `with-temp-buffer', so we explicitly ensure that happens, so that
processes will be invoked consistently.  BODY is as for that macro."
